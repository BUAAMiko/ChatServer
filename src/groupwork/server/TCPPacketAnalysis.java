        ...
        synchronized (MainService.db) {
            switch (s) {
                case "Ask_Message": {
                    ...
                    //获取好友
                    s = "SELECT DISTINCT `From`,`To` FROM ChatMessage WHERE `To` = " + map.get("Id") + " OR `From` = " + map.get("Id");
                    MainService.db.setSql(s);
                    List friendIdList = MainService.db.querySql();
                    List friend = new ArrayList();
                    for (int i = 0; i < friendIdList.size(); i++) {
                        Map tmp = (Map) friendIdList.get(i);
                        if (tmp.get("To").equals(map.get("Id")))
                            s = "SELECT Username FROM UserInfo WHERE Id = " + tmp.get("From");
                        else
                            s = "SELECT Username FROM UserInfo WHERE Id = " + tmp.get("To");
                        MainService.db.setSql(s);
                ...
                case "Send_Picture": {
                    ...
                    Functions.saveFile((byte[]) map.get("Data"),path);
                ...
                case "Ask_Picture": {
                    ...
                    m.put("File",f);
                    m.put("Data",Functions.fileToBytes(f));
                    m.put("Type",type);
                    response = Functions.objectToBytes(m);
                }
            }
